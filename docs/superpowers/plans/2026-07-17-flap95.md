...
> **Amendment (2026-07-17, user request):** Stakes may be placed in **USDm, USDC, or USDT** (whitelisted per-token in the contract; owner can add/remove). Both players stake the *same* token; tiers are 0.1 / 0.5 / 1 whole tokens scaled to the token's decimals (USDC/USDT use 6 decimals on Celo). `createDuel(token, stake)`, `DuelCreated` event carries the token, DB stores a `token` column, UI shows a currency picker and formats by token decimals. Gas feeCurrency inside MiniPay stays USDm. Celo mainnet: USDC `0xcebA9300f2b948710d2653dD7B07f33A8B32118C`, USDT `0x48065fbBE25f71C9282ddf5e1cD6D6A887483D5e`.
