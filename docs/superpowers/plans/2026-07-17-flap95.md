...
### Task 1: Scaffold web app + test tooling

**Files:**
- Create: `web/` (via create-next-app), `web/vitest.config.ts`, `web/.env.example`
- Modify: `web/package.json` (test script)

**Interfaces:**
- Produces: a running Next.js app in `web/` with `npm test` (vitest) green and deps installed: `wagmi viem @tanstack/react-query 98.css @neondatabase/serverless`, dev deps `vitest tsx`.

- [ ] **Step 1: Scaffold Next.js app**

```bash
cd ~/Desktop/celo-game
npx create-next-app@latest web --typescript --eslint --app --src-dir --no-tailwind --import-alias "@/*" --use-npm --yes
cd web
...
```bash
cd ~/Desktop/celo-game && git add web && git commit -m "chore: scaffold Next.js app with vitest and web3 deps"
```
...
**Files:**
- Create: `web/src/engine/rng.ts`, `web/src/engine/engine.ts`
- Modify: `web/src/engine/rng.test.ts` (replace smoke test)
- Test: `web/src/engine/engine.test.ts`
...
Replace `web/src/engine/rng.test.ts`:
```ts
import { describe, it, expect } from 'vitest';
...
Run: `cd web && npx vitest run src/engine/rng.test.ts` → Expected: FAIL (module not found).
...
```bash
cd web && npx tsx -e "import {simulate} from './src/engine/engine'; console.log(JSON.stringify({a: simulate(42, []), b: simulate(42, [5,15,25,35,45,60,75,90])}))"
```
...
```bash
cd ~/Desktop/celo-game && git add web/src/engine && git commit -m "feat: deterministic game engine with order-independent world generation"
```
...
**Files:**
- Create: `web/src/engine/verify.ts`
- Test: `web/src/engine/verify.test.ts`
...
```bash
cd ~/Desktop/celo-game && git add web/src/engine && git commit -m "feat: server-side trace validation and run verification"
```
...
**Files:**
- Create: `web/schema.sql`, `web/src/lib/db.ts`, `web/src/lib/duelStore.ts`
...
Provision a Neon Postgres via Vercel Marketplace (or neon.tech free tier), put the connection string in `web/.env.local` as `DATABASE_URL`, then:
...
Run: `cd web && npm run build` → Expected: success (store compiles).

```bash
cd ~/Desktop/celo-game && git add web/schema.sql web/src/lib && git commit -m "feat: Neon schema and duel store"
```
...
**Files:**
- Create: `web/src/lib/contracts.ts`, `web/src/lib/chain.ts`
- Create: `web/src/app/api/duels/route.ts`, `web/src/app/api/duels/[id]/route.ts`, `web/src/app/api/duels/[id]/bind/route.ts`, `web/src/app/api/duels/[id]/accept/route.ts`
...
- Produces (consumed by frontend Tasks 11–12):
  ...
  - `web/src/lib/contracts.ts` exports: `ESCROW_ADDRESS`, `USDM_ADDRESS`, `duelEscrowAbi`, `erc20Abi` (all `as const`)
  - `web/src/lib/chain.ts` exports: `publicClient` (viem, celo, forno)
...
`web/src/app/api/duels/route.ts`:
```ts
...
`web/src/app/api/duels/[id]/route.ts`:
```ts
import { NextResponse } from 'next/server';
...
`web/src/app/api/duels/[id]/bind/route.ts`:
```ts
import { NextResponse } from 'next/server';
...
`web/src/app/api/duels/[id]/accept/route.ts`:
```ts
import { NextResponse } from 'next/server';
import { parseEventLogs } from 'viem';
import { publicClient } from '@/lib/chain';
...
```bash
cd ~/Desktop/celo-game && git add web/src && git commit -m "feat: duel lifecycle API with on-chain event verification"
```
...
**Files:**
- Create: `web/src/lib/oracle.ts`, `web/src/app/api/duels/[id]/replay/route.ts`, `web/src/app/api/practice/route.ts`
- Test: `web/src/lib/oracle.test.ts`
...
`web/src/app/api/duels/[id]/replay/route.ts`:
```ts
import { NextResponse } from 'next/server';
...
`web/src/app/api/practice/route.ts`:
```ts
import { NextResponse } from 'next/server';
import { verifyRun } from '@/engine/verify';
...
Run: `cd web && npm test && npm run build` → Expected: all tests PASS, build succeeds.

```bash
cd ~/Desktop/celo-game && git add web/src && git commit -m "feat: replay verification, oracle settle relay, practice scores"
```
...
**Files:**
- Create: `web/src/lib/minipay.ts`, `web/src/lib/wagmi.ts`, `web/src/app/providers.tsx`, `web/src/components/Window.tsx`, `web/src/components/Dialog95.tsx`
...
`web/src/components/Dialog95.tsx`:
```tsx
'use client';
...
Replace `web/src/app/globals.css` content:
```css
...
```bash
cd ~/Desktop/celo-game && git add web/src && git commit -m "feat: Win95 shell, MiniPay detection, wagmi providers, home desktop"
```
...
**Files:**
- Create: `web/src/components/GameCanvas.tsx`, `web/src/app/play/page.tsx`, `web/src/app/fame/page.tsx`
...
```bash
cd ~/Desktop/celo-game && git add web/src && git commit -m "feat: game canvas with ghost support, practice mode, hall of fame"
```
...
**Files:**
- Create: `web/src/app/duels/new/page.tsx`
...
`web/src/app/duels/new/page.tsx`:
```tsx
'use client';
...
Run: `cd web && npm run build` → Expected: success. Full flow is exercised in Task 13 (needs deployed contract).

```bash
cd ~/Desktop/celo-game && git add web/src && git commit -m "feat: duel creation flow with approve, deposit, run recording"
```
...
**Files:**
- Create: `web/src/app/duels/page.tsx`, `web/src/app/duels/[id]/page.tsx`
...
```bash
cd ~/Desktop/celo-game && git add web/src && git commit -m "feat: open duels list, accept flow with ghost race, result dialog"
```
...
- Modify: `web/.env.local`, Vercel project env
...
- `web/.env.local` and Vercel project env (production): `DATABASE_URL`, `ORACLE_PRIVATE_KEY`, `NEXT_PUBLIC_ESCROW_ADDRESS=<deployed address>`, `NEXT_PUBLIC_USDM_ADDRESS=0x765DE816845861e75A25fCA122bb6898B8B1282a`.
...
cd web && npx vercel link && npx vercel --prod
```
Vercel settings: Root Directory = `web`. Expected: production URL live.

- [ ] **Step 5: Bundle size check**

Run: `cd web && npm run build` and read the route table output. Expected: every route's First Load JS **< 2MB** (typically ~150-300KB). If over, investigate imports before proceeding.
...
Write `README.md` (root): what Flap95 is (one-tap duels, ghost racing, Win95 UI), how it works (deterministic engine, oracle-signed settlement, escrow contract address + celoscan link), how to run locally (env vars table, `npm run dev`, `forge test`), path to the `isMiniPay()` hook (`web/src/lib/minipay.ts`) for Talent App data-source tracking, and a screenshot/GIF placeholder to fill after first deploy. Commit and push.
...
2. Register the project on talent.app for the current Proof of Ship campaign: GitHub URL, live URL, contract address, and the data-source path to `web/src/lib/minipay.ts`.
